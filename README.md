...
Opens the Cypress GUI during development

### `yarn cypress:run`

Runs Cypress CLI (in headless mode)

### `yarn cypress:start:app`

Runs React script, without opening up a browser (`BROWSER=none`) + adds necessary instrumentation for code analysis on the bundle (`-r @cypress/instrument-cra`)

### `yarn cypress:start:wait`

Starts the app + waits for [http://localhost:3000](http://localhost:3000) to become available, then Runs Cypress CLI

### `yarn cypress:open-ct`

Opens the Cypress Unit Test in the GUI during development
